...
BotScoredData = namedtuple(
    "BotScoredData", ["name", "num_moves", "num_collisions", "score"]
)
...
for bot in bots:
    bot_scores.append(
        BotScoredData(
            bot.name,
            move_count[bot.name],
            collision_count[bot.name],
            move_count[bot.name] + collision_count[bot.name],
        )
    )

## --- DISPLAYING LIVE RACE -- ##
